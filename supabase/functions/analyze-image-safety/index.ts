...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { imageUrl, analysisType, sensitivity = 3 } = await req.json();

    if (!imageUrl) {
      throw new Error('Image URL is required');
    }

    console.log('🔍 Image safety analysis request:', {
      userId: user.id,
      analysisType,
      sensitivity,
    ...
    // Prepare OpenAI Vision API request for safety analysis
    const systemPrompt = `You are a content safety analyzer. Analyze images for potential policy violations that might be rejected by AI content generation services.
...
Respond with valid JSON only:
{
  ...
  "suggestions": ["how to make it safer"]
    ...
    const userPrompt = "Analyze this image for content policy violations that might cause AI generation to fail.";
    ...
    console.log('📡 Calling OpenAI Vision API for safety analysis...');
    ...
    try {
      ...
      // Fallback parsing
      parsedAnalysis = {
        safe: !analysisText.toLowerCase().includes('unsafe') && !analysisText.toLowerCase().includes('violation'),
        confidence: 0.5,
        violations: [],
        reasoning: 'Could not parse detailed analysis',
        suggestions: ['Review content for potential policy violations']
      };
    }
    ...
    const result = {
      ...
      analysisType: 'image_safety',
      sensitivity: sensitivity,
      timestamp: new Date().toISOString(),
      rawAnalysis: analysisText
    };

    console.log('🛡️ Image safety analysis result:', {
      safe: result.safe,
    ...
    console.error('❌ Error in analyze-image-safety:', error);
