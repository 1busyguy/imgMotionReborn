...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🎬 Omnihuman generation request:', {
      generationId,
    ...
    if (!generationId || !imageUrl || !audioUrl) {
      throw new Error('Missing required parameters: generationId, imageUrl, and audioUrl are required');
    }
    ...
    // Prepare FAL.ai API request for Omnihuman
    const falParams = {
      image_url: imageUrl,
      prompt_extend: false,
      audio_url: audioUrl
    ...
    const queueUrl = `https://queue.fal.run/fal-ai/bytedance/omnihuman?fal_webhook=${encodeURIComponent(webhookUrl)}`;
    ...
    console.log('✅ Omnihuman request queued successfully, webhook will handle completion');
    ...
    return new Response(JSON.stringify({
      ...
      message: 'Omnihuman talking avatar generation queued successfully. Webhook will update when complete.',
      fal_request_id: requestId,
      webhook_url: webhookUrl,
    ...
    console.error('❌ Error in fal-omnihuman:', error);
