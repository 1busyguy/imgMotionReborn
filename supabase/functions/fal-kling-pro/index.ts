...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🎬 Kling Pro generation request:', {
      generationId,
    ...
    // Prepare FAL.ai API request for Kling Pro
    const falParams = {
      image_url: imageUrl,
      prompt: prompt.trim(),
      prompt_extend: false,
    ...
    if (cfgScale !== undefined && cfgScale !== null) {
      falParams.cfg_scale = Math.max(0.0, Math.min(1.0, cfgScale));
    }
    if (seed && seed > 0) {
      falParams.seed = seed;
    }
    ...
    const queueUrl = `https://queue.fal.run/fal-ai/kling-video/v2.1/pro/image-to-video?fal_webhook=${encodeURIComponent(webhookUrl)}`;
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        metadata: {
          ...
          tool_type: 'kling-pro-video',  // This will be the folder name in storage
    ...
    console.log('✅ Kling Pro request queued successfully, webhook will handle completion');
    ...
    return new Response(JSON.stringify({
      ...
      message: 'Kling Pro video generation queued successfully. Webhook will update when complete.',
      fal_request_id: requestId,
      webhook_url: webhookUrl,
    ...
    console.error('❌ Error in fal-kling-pro:', error);
