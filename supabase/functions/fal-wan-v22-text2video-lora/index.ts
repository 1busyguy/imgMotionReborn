...
serve(async (req) => {
    ...
    try {
        ...
        console.log('🎬 WAN v2.2 Text2Video LoRA generation request:', {
            generationId,
            prompt: prompt?.substring(0, 50) + '...',
            resolution,
            hasLoRAs: loras.length > 0
        });
        ...
        // Prepare FAL.ai parameters for WAN v2.2-a14b Text-to-Video LoRA
        const falParams = {
            prompt: prompt.trim(),
        ...
        // CORRECT WEBHOOK URL CONSTRUCTION (copied from working file)
        const webhookUrl = getWebhookUrl();
        console.log('🔗 Webhook URL:', webhookUrl);

        // Use FAL.ai queue system with webhook
        const queueUrl = `https://queue.fal.run/fal-ai/wan/v2.2-a14b/text-to-video/lora?fal_webhook=${encodeURIComponent(webhookUrl)}`;
        ...
        const { error: updateError } = await supabase
            ...
            .update({
                metadata: {
                    ...
                    model: 'wan-v2.2-a14b-text2video-lora',
                    tool_type: 'wan-v22-text2video-lora',
        ...
        return new Response(JSON.stringify({
            ...
            message: 'WAN v2.2 Text2Video LoRA generation queued successfully',
            fal_request_id: requestId,
            webhook_url: webhookUrl,
