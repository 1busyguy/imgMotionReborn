...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🎨 Qwen Image-to-Image generation request:', {
      generationId,
    ...
    if (!generationId || !prompt?.trim() || !image_url) {
      throw new Error('Missing required parameters: generationId, prompt, and image_url are required');
    }
    ...
    // Prepare FAL.ai API request for Qwen Image-to-Image
    const falParams = {
      prompt: prompt.trim(),
      image_url: image_url,
      strength: Math.max(0.1, Math.min(1.0, strength)),
      image_size: image_size,
      num_inference_steps: Math.max(20, Math.min(100, num_inference_steps)),
      guidance_scale: Math.max(1.0, Math.min(20.0, guidance_scale)),
      num_images: Math.max(1, Math.min(4, num_images)),
    ...
    const queueUrl = `https://queue.fal.run/fal-ai/qwen-image/image-to-image?fal_webhook=${encodeURIComponent(webhookUrl)}`;
    ...
    console.log('✅ Qwen Image-to-Image request queued successfully, webhook will handle completion');
    ...
    return new Response(JSON.stringify({
      ...
      message: 'Qwen Image-to-Image generation queued successfully. Webhook will update when complete.',
      fal_request_id: requestId,
    ...
    console.error('❌ Error in fal-qwen-image-to-image:', error);
