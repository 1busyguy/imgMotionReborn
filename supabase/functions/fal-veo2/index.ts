...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🎬 VEO2 generation request:', {
      generationId,
    ...
    // Prepare FAL.ai API request for VEO2
    const falParams = {
      prompt: prompt.trim(),
      image_url: imageUrl,
      prompt_extend: false,
    ...
    const queueUrl = `https://queue.fal.run/fal-ai/veo2/image-to-video?fal_webhook=${encodeURIComponent(webhookUrl)}`;
    ...
    console.log('✅ VEO2 request queued successfully, webhook will handle completion');
    ...
    return new Response(JSON.stringify({
      ...
      message: 'VEO2 video generation queued successfully. Webhook will update when complete.',
      fal_request_id: requestId,
    ...
    console.error('❌ Error in fal-veo2:', error);
