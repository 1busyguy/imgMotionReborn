...
serve(async (req) => {
  ...
  try {
    ...
    // Prepare FAL.ai API request for Qwen Image
    const falParams = {
      prompt: prompt.trim(),
    ...
    const queueUrl = `https://queue.fal.run/fal-ai/qwen-image?fal_webhook=${encodeURIComponent(webhookUrl)}`;
    ...
    console.log('✅ Qwen Image request queued successfully, webhook will handle completion');
    ...
    return new Response(JSON.stringify({
      ...
      message: 'Qwen Image generation queued successfully. Webhook will update when complete.',
      fal_request_id: requestId,
    ...
    console.error('❌ Error in fal-qwen-image:', error);
