...
function shouldWatermarkContent(profile) {
  ...
  const tier = profile.subscription_tier || profile.subscription_status || profile.tier || profile.plan || 'free';
...
async function processVideoWithFFmpeg(generation, videoUrl, userProfile) {
  ...
  const isFreeTier = !userProfile || !userProfile.subscription_tier || userProfile.subscription_tier.toLowerCase() === 'free' || userProfile.subscription_tier.toLowerCase() === 'trial';
  ...
  const hasFalThumbnail = generation.metadata?.thumbnail_url || generation.metadata?.original_thumbnail_url || generation.thumbnail_url;
  if (!hasFalThumbnail) {
    console.log('📸 No FAL thumbnail found, extracting with FFmpeg...');
    tasks.push(callFFmpegService('/api/v1/extract-thumbnail', {
  ...
  if (isFreeTier) {
    ...
    tasks.push(callFFmpegService('/api/v1/add-watermark', {
      generation_id: generation.id,
      video_url: videoUrl,
      user_id: generation.user_id,
...
function handleWebhookError(event, generation) {
  ...
  if (!contentViolation && violationKeywords.some((keyword)=>errorMessage.toLowerCase().includes(keyword))) {
    contentViolation = true;
  }
...
serve(async (req)=>{
  ...
  try {
    ...
    if (tempEvent && (tempEvent.request_id || tempEvent.gateway_request_id)) {
      ...
      const { data: generation } = await supabase.from('ai_generations').select('metadata').eq('metadata->>fal_request_id', tempRequestId).maybeSingle();
    ...
    const { data: generation, error: findError } = await supabase.from('ai_generations').select('id, user_id, metadata, status, tool_type').eq('metadata->>fal_request_id', requestId).eq('status', 'processing').maybeSingle();
    ...
    if (event.status === "OK" || event.status === "COMPLETED" || event.status === "SUCCESS") {
      ...
      if (event.payload?.video?.url) {
        ...
        thumbnailUrl = event.payload?.video?.preview?.url || event.payload?.video?.thumbnail?.url || event.payload?.preview?.url || event.payload?.thumbnail?.url || event.payload?.first_frame?.url;
      } else if (typeof event.payload?.video === 'string') {
        outputUrl = event.payload.video;
        // Check for thumbnail in other payload locations
        thumbnailUrl = event.payload?.preview?.url || event.payload?.thumbnail?.url || event.payload?.first_frame?.url;
      ...
      if (event.payload?.images && Array.isArray(event.payload.images)) {
          ...
          if (imgUrl) {
            try {
              ...
              if (response.ok) {
                ...
                const { error: uploadError } = await supabase.storage.from('user-files').upload(filePath, fileData, {
        ...
        try {
          ...
          if (event.payload?.image || generation.tool_type?.toLowerCase().includes('image') || generation.tool_type?.toLowerCase().includes('flux') || generation.tool_type?.toLowerCase().includes('bria') || generation.tool_type?.toLowerCase().includes('hidream')) {
          ...
          } else if (event.payload?.audio || generation.tool_type?.toLowerCase().includes('audio') || generation.tool_type?.toLowerCase().includes('music') || generation.tool_type?.toLowerCase().includes('sound')) {
            fileExtension = 'mp3';
        ...
        if (thumbnailUrl) {
          try {
            ...
            if (thumbnailResponse.ok) {
              ...
              const { error: thumbnailUploadError } = await supabase.storage.from('user-files').upload(thumbnailPath, thumbnailData, {
                contentType: 'image/jpeg',
                upsert: true
              });
              if (!thumbnailUploadError) {
                const { data: { publicUrl: thumbnailPublicUrl } } = supabase.storage.from('user-files').getPublicUrl(thumbnailPath);
