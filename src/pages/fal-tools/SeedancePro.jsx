...
const SeedancePro = () => {
  ...
  // Configuration state - based on Seedance Pro API
  ...
  const durationOptions = [
    { label: '3 seconds', value: "3", cost: 72 },
    { label: '4 seconds', value: "4", cost: 96 },
    { label: '5 seconds', value: "5", cost: 120 },
    { label: '6 seconds', value: "6", cost: 144 },
    { label: '7 seconds', value: "7", cost: 168 },
    { label: '8 seconds', value: "8", cost: 192 },
    { label: '9 seconds', value: "9", cost: 216 },
    { label: '10 seconds', value: "10", cost: 240 },
    { label: '11 seconds', value: "11", cost: 264 },
    { label: '12 seconds', value: "12", cost: 288 }
  ];
  ...
  useEffect(() => {
    if (user) {
      ...
      const subscription = supabase
        .channel('seedance_pro_generations')
        .on(
          'postgres_changes',
          {
            event: '*',
            ...
            filter: `user_id=eq.${user.id},tool_type=eq.fal_seedance_pro`
          },
          (payload) => {
      ...
      console.log('Real-time subscription set up for Seedance Pro');
  ...
  const handleRealtimeUpdate = (payload) => {
    ...
    if (newRecord?.status === 'failed') {
      console.log('Video generation failed:', newRecord.error_message);
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      const { url } = await uploadFile(file, 'seedance-pro-images');
  ...
  const calculateTokenCost = () => {
    // Base cost calculation: 24 tokens per second
    let baseCost = parseInt(config.duration) * 24;
    
    // Apply 25% discount for 480p
    if (config.resolution === '480p') {
      baseCost = Math.round(baseCost * 0.75);
    }
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (isNSFWError(error.message)) {
        ...
        showAlert('error', 'Generation Failed', `Seedance Pro generation failed: ${error.message}`);
      }
    } finally {
      setGenerating(false);
    }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-xl font-bold text-white">Seedance Pro Video</h1>
                  <p className="text-purple-200 text-sm">Professional image-to-video generation with Seedance Pro technology</p>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div 
                    ...
                    onDragOver={(e) => {
                      e.preventDefault();
                      e.currentTarget.classList.add('border-teal-400', 'bg-teal-500/10');
                    }}
                    onDragLeave={(e) => {
                      e.preventDefault();
                      e.currentTarget.classList.remove('border-teal-400', 'bg-teal-500/10');
                    }}
                    onDrop={(e) => {
                      e.preventDefault();
                      e.currentTarget.classList.remove('border-teal-400', 'bg-teal-500/10');
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between mt-1">
                    <p className="text-purple-300 text-xs">
                      {imageAnalysis ? "✨ AI-generated motion prompt" : "Be specific about the motion, camera movement, and scene dynamics"}
                    </p>
                    {imageAnalysis && (
                      <button
                ...
                <div>
                  <label className="flex items-center space-x-3">
                    <input
                      ...
                      onChange={(e) => setConfig(prev => ({ ...prev, cameraFixed: e.target.checked }))}
                    ...
                    <div>
                      ...
                      <p className="text-purple-300 text-xs">
                        Keep camera position static (no camera movement)
                      </p>
                    </div>
                  </label>
                </div>
                ...
                <div className="bg-teal-500/10 border border-teal-500/30 rounded-lg p-3">
                  <p className="text-teal-200 text-xs">
                    ...
                    Seedance Pro generation may take 3-8 minutes. Cost: 24 tokens per second (480p/1080p available).
                  </p>
                </div>
              </div>
            </div>
                      ...
                      <div key={generation.id} className="bg-white/5 rounded-lg p-4">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-purple-300 text-xs mt-1">
                              Seedance Pro generation typically takes 3-8 minutes
                            </p>
                          </div>
                          <div className="flex items-center space-x-3">
                      ...
                      >
                        <div className="p-4">
                          ...
                          {generation.status === 'processing' && (
                            <div className="mb-4 bg-gradient-to-br from-teal-500/20 to-cyan-500/20 rounded-lg p-8 text-center">
                              <RefreshCw className="w-12 h-12 text-teal-300 animate-spin mx-auto mb-2" />
                              <p className="text-teal-200">Generating professional video...</p>
                              <p className="text-teal-300 text-sm">This may take 3-8 minutes</p>
                            </div>
                          )}
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-white">{selectedGeneration.input_data?.duration}s</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-white">{selectedGeneration.input_data?.cameraFixed ? 'Fixed' : 'Dynamic'}</span>
                    </div>
