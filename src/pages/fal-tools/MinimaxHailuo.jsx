...
const MinimaxHailuo = () => {
  ...
  // Configuration state - based on API parameters
  ...
  useEffect(() => {
    if (user) {
      ...
      const subscription = supabase
        .channel('minimax_hailuo_generations')
        .on(
          'postgres_changes',
          {
            event: '*',
            ...
            filter: `user_id=eq.${user.id},tool_type=eq.fal_minimax_hailuo`
          },
          (payload) => {
      ...
      console.log('Real-time subscription set up for Minimax Hailuo');
  ...
  const fetchGenerations = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('user_id', user.id)
        .eq('tool_type', 'fal_minimax_hailuo')
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      const { url } = await uploadFile(file, 'video-source-images');
  ...
  const calculateTokenCost = () => {
    ...
    if (config.duration === "10") {
      return 90; // 10 seconds = 90 tokens
    } else {
      return 50; // 6 seconds = 50 tokens (default)
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    setGenerating(true);
    try {
      console.log('Starting Minimax Hailuo generation...');
      ...
      // Call Edge Function
      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/fal-minimax-hailuo`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${(await supabase.auth.getSession()).data.session.access_token}`,
          'Content-Type': 'application/json'
  ...
  const handleDownload = async (videoUrl) => {
    try {
      ...
      link.href = url;
      link.download = `minimax-hailuo-${Date.now()}.mp4`;
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-xl font-bold text-white">Minimax Hailuo Video</h1>
                  <p className="text-purple-200 text-sm">Transform images into cinematic videos with AI</p>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {durationOptions.map((option) => (
                      ...
                      >
                        {option.label} ({option.value === "10" ? "90" : "50"} tokens)
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  <label className="flex items-center space-x-3">
                    <input
                      type="checkbox"
                      checked={config.promptOptimizer}
                      onChange={(e) => setConfig(prev => ({ ...prev, promptOptimizer: e.target.checked }))}
                ...
                <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-3">
                  <p className="text-blue-200 text-xs">
                    ...
                    Video generation may take 2-5 minutes. You can leave this page and check your Gallery later.
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6">
                ...
                {generations.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-purple-300 text-sm">Upload an image and describe motion to create videos</p>
                  </div>
                ) : (
                  <div className="space-y-6">
                      ...
                      >
                        <div className="p-4">
                          ...
                          {generation.status === 'processing' && (
                            <div className="mb-4 bg-gradient-to-br from-blue-500/20 to-cyan-500/20 rounded-lg p-8 text-center">
                              <RefreshCw className="w-12 h-12 text-blue-300 animate-spin mx-auto mb-2" />
                              <p className="text-blue-200">Generating video...</p>
                              <p className="text-blue-300 text-sm">This may take 2-5 minutes</p>
                            </div>
                          )}
                          ...
                          <div className="space-y-2 text-sm text-purple-300">
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <span>
                                <strong>Optimizer:</strong> {generation.input_data?.promptOptimizer ? 'Enabled' : 'Disabled'}
                              </span>
                            </div>
                          </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      <span className="text-purple-200">Prompt Optimizer:</span>
                      <span className="text-white">{selectedGeneration.input_data?.promptOptimizer ? 'Enabled' : 'Disabled'}</span>
                    </div>
