...
const VEO3Standard = () => {
    ...
    // Configuration state - based on VEO3 Standard API parameters
    ...
    useEffect(() => {
        if (user) {
            ...
            const subscription = supabase
                .channel('veo3_standard_generations')
                .on(
                    'postgres_changes',
                    {
                        event: '*',
                        ...
                        filter: `user_id=eq.${user.id},tool_type=eq.fal_veo3`
                    },
                    (payload) => {
                        console.log('🎬 VEO3 Standard Real-time update received:', payload);
                        handleRealtimeUpdate(payload);
                    }
                )
                .subscribe();

            console.log('Real-time subscription set up for VEO3 Standard');
                ...
                if (newRecord.metadata?.error_type === 'content_violation') {
                    ...
                    setTimeout(() => {
                        setNsfwError({
                            ...
                            technical: newRecord.metadata?.error_details || newRecord.metadata?.error_message
                        });
                        setShowNSFWAlert(true);
                    }, 500);
                } else {
                    // Show general error with delay
                    setTimeout(() => {
                        const errorMessage = newRecord.metadata?.error_message || newRecord.error_message || 'Generation failed';
                        showAlert('error', 'Generation Failed', `VEO3 Standard generation failed: ${errorMessage}`);
                    }, 1000);
                }
            }
        }
    ...
    const handleImageUpload = async (event) => {
        ...
        try {
            const { url } = await uploadFile(file, 'veo3-standard-images');
    ...
    const handleGenerate = async () => {
        ...
        try {
            console.log('🚀 Starting VEO3 Standard generation...');
            ...
            const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/fal-veo3`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${(await supabase.auth.getSession()).data.session.access_token}`,
                    'Content-Type': 'application/json'
            ...
            if (!responseData.success) {
                ...
                if (responseData.error_type === 'content_violation') {
                    ...
                    // Force refresh generations after a moment
                    setTimeout(async () => {
                        await fetchGenerations();
                    }, 500);
                } else {
                    // Other errors - show general alert
            ...
            console.log('✅ VEO3 Standard generation request submitted successfully');
            ...
            // This catch block is now for unexpected errors only
    ...
    const handleDownload = async (videoUrl) => {
        try {
            ...
            link.href = url;
            link.download = `veo3-standard-video-${Date.now()}.mp4`;
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-4">
                            ...
                            <div className="flex items-center space-x-3">
                                ...
                                <div>
                                    <h1 className="text-xl font-bold text-white">VEO3 Standard</h1>
                                    <p className="text-purple-200 text-sm">High-quality image-to-video generation with Google's VEO3 model</p>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div className="grid grid-cols-1 gap-2">
                                        {resolutionOptions.map((option) => (
                                            <button
                                                ...
                                                className={`px-3 py-2 rounded-lg text-sm font-medium transition-all ${config.resolution === option.value
                                            ...
                                            <div key={generation.id} className="bg-white/5 rounded-lg p-4">
                                                <div className="flex items-center justify-between">
                                                    <div>
                                                        ...
                                                        <p className="text-purple-300 text-xs mt-1">
                                                            VEO3 Standard generation typically takes 5-12 minutes
                                                        </p>
                                                    </div>
                                                    <div className="flex items-center space-x-3">
                            ...
                            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6">
                                ...
                                {generations.length === 0 ? (
                                    <div className="text-center py-12">
                                        ...
                                        <p className="text-purple-300 text-sm">Upload an image and describe motion to create high-quality videos with VEO3</p>
                                    </div>
                                ) : (
                                    <div className="space-y-6">
                                                ...
                                                <div className="p-4">
                                                    ...
                                                    {generation.status === 'processing' && (
                                                        <div className="mb-4 bg-gradient-to-br from-violet-500/20 to-purple-500/20 rounded-lg p-8 text-center">
                                                            ...
                                                            <p className="text-violet-200">Generating high-quality video...</p>
                                                            <p className="text-violet-300 text-sm">This may take 5-12 minutes</p>
                                                        </div>
                                                    )}
                                                    ...
                                                    {generation.status === 'failed' && (
                                                        <GenerationError
                                                            ...
                                                            canRetry={generation.metadata?.error_type !== 'content_violation'}
                                                        />
                                                    )}
