...
const KlingPro = () => {
  ...
  // Configuration state - based on Kling Pro API
  ...
  const durationOptions = [
    { label: '5 seconds', value: "5", cost: 90 },
    { label: '10 seconds', value: "10", cost: 180 }
  ];
  ...
  useEffect(() => {
    if (user) {
      ...
      const subscription = supabase
        ...
        .on(
          'postgres_changes',
          {
            ...
            filter: `user_id=eq.${user.id},tool_type=eq.fal_kling_pro`
          },
          (payload) => {
      ...
      console.log('Real-time subscription set up for Kling Pro');
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      const { url } = await uploadFile(file, 'kling-pro-images');
  ...
  const calculateTokenCost = () => {
    ...
    if (config.duration === "10") {
      return 180; // 10 seconds = 180 tokens
    } else {
      return 90; // 5 seconds = 90 tokens (default)
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    setGenerating(true);
    try {
      console.log('Starting Kling Pro generation...');
      ...
      // Call Edge Function using direct fetch (like other successful tools)
      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/fal-kling-pro`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${(await supabase.auth.getSession()).data.session.access_token}`,
          'Content-Type': 'application/json'
  ...
  const handleDownload = async (videoUrl) => {
    try {
      ...
      link.href = url;
      link.download = `kling-pro-video-${Date.now()}.mp4`;
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-xl font-bold text-white">Kling Pro Video</h1>
                  <p className="text-purple-200 text-sm">Professional-grade image-to-video with Kling v2.1 Pro</p>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setConfig(prev => ({ ...prev, cfgScale: parseFloat(e.target.value) }))}
                ...
                <div className="bg-orange-500/10 border border-orange-500/30 rounded-lg p-3">
                  <p className="text-orange-200 text-xs">
                    ...
                    Kling Pro video generation may take 3-8 minutes. You can navigate away and check your Gallery later.
                  </p>
                </div>
              </div>
            </div>
                      ...
                      <div key={generation.id} className="bg-white/5 rounded-lg p-4">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-purple-300 text-xs mt-1">
                              Kling Pro generation typically takes 3-8 minutes
                            </p>
                          </div>
                          <div className="flex items-center space-x-3">
                        ...
                        <div className="p-4">
                          ...
                          {generation.status === 'processing' && (
                            <div className="mb-4 bg-gradient-to-br from-orange-500/20 to-red-500/20 rounded-lg p-8 text-center">
                              ...
                              <p className="text-orange-200">Generating professional video...</p>
