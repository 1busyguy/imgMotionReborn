  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        ...
        // Removed Google OAuth redirect that was causing infinite loop
          console.log('📍 Capturing IP address for new user...');
          
          // First capture the IP in the user's profile
          const captureResponse = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/capture-signup-ip`, {
        ...
        if (session?.user) {
          ...
          if (captureResponse.ok) {
            console.log('✅ IP captured in profile');
          } else {
            console.warn('⚠️ IP capture failed:', await captureResponse.text());
          }
          
          // Then increment the IP signup count
          const incrementResponse = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/increment-ip-signup`, {
          });
          ...
          if (incrementResponse.ok) {
            console.log('✅ IP signup count incremented');
          } else {
            console.warn('⚠️ IP increment failed:', await incrementResponse.text());
          }
          
          console.log('✅ Complete IP tracking finished for email signup');
