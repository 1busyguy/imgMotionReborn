...
import { 
  ArrowLeft, 
  Zap, 
  ${toolData.category === 'video' ? 'Video' : toolData.category === 'audio' ? 'Music' : 'Image'}, 
  Upload, 
  Download, 
  Trash2, 
  RefreshCw,
...
${parameters.map(param => `    ${param.name}: ${JSON.stringify(param.defaultValue)}`).join(',\n')}
  });
  ...
  useEffect(() => {
    if (user) {
      ...
      const subscription = supabase
        .channel('${toolData.toolType}_generations')
        .on(
          'postgres_changes',
          {
            event: '*',
            ...
            filter: \`user_id=eq.\${user.id},tool_type=eq.${toolData.toolType}\`
          },
          (payload) => {
